...
server <- function(input, output, session) {
  ...
  alpha_a1 <- eventReactive(input$Simulate, {Alpha()$sen})
  alpha_b1 <- eventReactive(input$Simulate, {Alpha()$spe})
  
  Beta <- eventReactive(input$Simulate, {estimate_beta(c(input$beta_l, input$beta1, input$beta_u))})
  
  beta_a1 <- eventReactive(input$Simulate, {Beta()$sen})
  beta_b1 <- eventReactive(input$Simulate, {Beta()$spe})
  
  alpha_d <- eventReactive(input$Simulate,
                           {rbeta(input$n_iter1, alpha_a1(), alpha_b1())
                           })
  beta_d  <- eventReactive(input$Simulate,
                           {rbeta(input$n_iter1, beta_a1(), beta_b1())
                           })
